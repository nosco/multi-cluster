...
var MultiCluster = function(appPath) {
    cluster.setupMaster();
...
MultiCluster.prototype.startWorker = function(cpu) {
    var self = this;

    cluster.settings.exec = this.appPath;
    
    var worker = cluster.fork({ WORKER: '1', WORKER_PROCESS: cpu });
    ...
    worker.on('disconnect', function() {
            var worker_process = workerPidToId[worker.process.pid];
            ...
            self.startWorker(worker_process);
        });
